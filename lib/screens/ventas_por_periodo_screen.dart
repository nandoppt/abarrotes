...
class VentasPorPeriodoScreen extends StatelessWidget {
  final String clave; // ej: "2025-05" o "2025-05-20" (inicio de semana)
  final String tipo;  // 'semana' o 'mes'

  const VentasPorPeriodoScreen({super.key, required this.clave, required this.tipo});
  ...
  Widget build(BuildContext context) {
    ...
    for (var venta in ventas) {
      ...
      bool coincide = false;
      if (tipo == 'semana') {
        final inicioSemana = DateTime.parse(clave);
        final finSemana = inicioSemana.add(const Duration(days: 6));
        coincide = !fecha.isBefore(inicioSemana) && !fecha.isAfter(finSemana);
      } else {
        // clave viene como '2025-07' para julio 2025, por ejemplo
        final partes = clave.split('-');
        final int anio = int.parse(partes[0]);
        final int mes = int.parse(partes[1]);

        final inicioMes = DateTime(anio, mes, 1, 0, 0, 0);
        final finMes = mes < 12
            ? DateTime(anio, mes + 1, 1, 0, 0, 0).subtract(const Duration(seconds: 1))
            : DateTime(anio + 1, 1, 1, 0, 0, 0).subtract(const Duration(seconds: 1));
        coincide = !fecha.isBefore(inicioMes) && !fecha.isAfter(finMes);
      }

      if (coincide) {
        ventasFiltradas.add(venta);
        totalPeriodo += (venta['total'] ?? 0.0) as double;
      }
    }

    final titulo = tipo == 'semana'
          ...
          children: [
                    ...
                    children: [
                      ...
                      Column(
                        ...
                        children: [
                          ...
                          Text('\$${totalPeriodo.toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 22, color: Colors.green)),
                        ],
                      ),
                    ],
                  ),
            ...
            Expanded(
              child: ventasFiltradas.isEmpty
                  ? Center(child: Text('No hay ventas para este periodo', style: TextStyle(color: Colors.grey[600])))
                  : ListView.builder(
                ...
                children: [
                  Expanded(
                    child: ElevatedButton.icon(
                      onPressed: () => _exportarExcel(context, ventasFiltradas, titulo),
                  ...
                  Expanded(
                    child: ElevatedButton.icon(
                      onPressed: () => _generarPdf(context, ventasFiltradas, titulo),
